  ...
  return (
    <SectionShell id="statistics" className="pt-0">
      <div className="border-y border-border py-5 md:py-6">
        <div
          className={cn(
            "grid auto-rows-fr items-stretch gap-0",
            collectionClassName,
          )}
        >
          {content.map((item, index) => (
            <Reveal key={item.id} delay={index * 0.05} className="h-full">
              <Card
                interactive
                className={`flex h-full flex-col rounded-none border-0 bg-transparent p-5 shadow-none before:hidden hover:bg-white/[0.025] hover:shadow-none md:p-6 ${index > 0 ? "md:border-s md:border-border md:ps-6" : ""}`}
              >
